...
class ToyMPNN(nn.Module):
    def __init__(self, CONV_OP, nlayer, inch, outch, hidch, is_graph_level=False, pedim=0, trans_nhead=8, trans_nlayer=2, wo_dee=False, edge_dim=None, *args, **kwargs) -> None:
        ...
        if self.CONV_OP == GINConv:
            ...
            # self.dec = MLP_batchnorm(2*hidch, outch, 2*hidch, num_hidden=2)
...
class ToyMDNN(nn.Module):
    def __init__(self, CONV_OP, nlayer, inch, outch, hidch, is_graph_level=False, pedim=0, trans_nhead=4, trans_nlayer=2, wo_dee=False, edge_dim=None, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.CONV_OP = CONV_OP
        if self.CONV_OP == GINConv:
            self.enc = CONV_OP(MLP_batchnorm(inch, hidch, hidch))
    ...
    def mdnn_forward(self, x, edge_index0, de_x, skip_connect=True, batch=None):
        trans_x, pe, dee, ID, pad_mask, batch_size = de_x
        pe = self.pe_lin(pe)
        if self.wo_dee:
            dee = None
        else:
        ...
        if self.CONV_OP == GCN2Conv:
            ...
            x = torch.cat([self.enc(x, edge_index0), self.enc(x, edge_index0)], -1)
        x0 = x
        x00 = x0[..., :int(x.shape[-1]/2)]
        trans_x = self.transformer(trans_x, pad_mask, [pe, dee, ID]) # x0[..., :int(x0.shape[-1]/2)] + 
        x00[:batch_size] = trans_x
        for i, net in enumerate(self.net):
            edge_index = edge_index0
            if self.CONV_OP == GCN2Conv:
                x = torch.cat([net(x, x0, edge_index), x00], -1)
            else:
                x = torch.cat([net(x, edge_index), x00], -1)
            x = torch.relu(x)
            if skip_connect:
                x = x0 + x
        ## Predict head
...
class MLP_batchnorm(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_hidden=1, dropout=0.1, output_activation='relu',batchnorm_affine=True, device='cpu', dtype=torch.float32):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for layer in range(self.num_hidden):
            x = torch.dropout(self.linears[layer](x), p=self.dropout, train=self.training)
            x = self.bns[layer](x)
            x = self.activation(x)
